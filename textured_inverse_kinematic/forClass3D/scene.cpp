...
scene::scene()
{
	...
	this->data = stbi_load("./res/textures/mouse3.jpg", &width, &height, &numComponents, 3);
	this->data1 = stbi_load("./res/textures/cat3.jpg", &width1, &height1, &numComponents1, 3);
...
std::vector<glm::mat4> scene::draw(int numshader,Display* display,glm::mat4 M ,glm::mat4 P) {
	...
	glm::mat4 m=glm::rotate(-90.0f, glm::vec3(1, 0, 0))*glm::rotate(rotate_x,glm::vec3(0,0,1))*glm::rotate(rotate_y,glm::vec3(1,0,0));
	if (numshader == 0)
	{
		glm::mat4 mvp = P*m;
		shader->Bind();
	...
	glm::mat4 MVP = P*m*mov->getTranslateafter()*glm::translate(glm::mat4(), glm::vec3(0, 0, 0))*mov->getRotate()*mov->getpsi()*mov->gettheta()*glm::translate(glm::mat4(), glm::vec3(0, 0, 0));
	
	this->cubepos = moveafter*bigphi*bigtheta* scale;
//	glm::translate(glm::mat4(), glm::vec3(0, -1, 0))*mov->getTranslateafter()*glm::translate(glm::mat4(), glm::vec3(0, 0, 1))*mov->getRotate()*mov->getpsi()*mov->gettheta()*mov->getphi()*glm::translate(glm::mat4(), glm::vec3(0, 0, -1))*mov->getTranslatebefor();
	if (numshader == 0) {
		shader->texture(data, width, height,textures[0]);
		text = true;
	}
	shader->Bind();
	...
	for (unsigned int i = 0; i < cubearray->size(); i++) {
		...
		if (numshader == 0) {
			shader->texture(data1, width1, height1, textures[0]);
			text = true;
		}
		shader->Bind();
